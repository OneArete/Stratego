...
import { adaptationPatternCandidates,adaptationPatternSummary,adaptationPatternAudit,applyAdaptationPatternReview,mergeAdaptationPatternReviews,adaptationPatternReviewAudit,adaptationPatternReviewSummary,reconcileAdaptationPatternReviews } from './core/adaptation-patterns.js';
...
function understanding(){
  ...
  const adaptationPatterns=mergeAdaptationPatternReviews(rawAdaptationPatterns,state.adaptationPatternReviews);
  ...
  shell(`${header('UNDERSTANDING')}<section class="stack understanding-screen"><div><p class="eyebrow">WHAT I CURRENTLY UNDERSTAND ABOUT YOU</p><h2>A council that accumulates experience.</h2><p class="muted">Each Advisor learns only from reflections in its own domain. Their uncertainty remains visible and correctable.</p><button class="context-help inline" data-guide-section="understanding">How should I read Understanding?</button></div>
    ...
    <section class="adaptation-pattern-governance understanding-group is-hidden" data-understanding-group="agency">
      ...
      ${adaptationPatterns.length?adaptationPatterns.map(pattern=>`<article class="adaptation-pattern-item ${esc(pattern.reviewStatus)}"><div><strong>${esc(pattern.practiceId)}</strong><span>${esc(pattern.reviewStatus)}</span></div><p>${esc(adaptationPatternSummary(pattern))}</p><small>${pattern.uniqueSessions} distinct sessions · ${esc(pattern.evidenceBoundary)}</small><small>${esc(adaptationPatternReviewSummary(pattern))}</small><div class="insight-actions">${pattern.reviewStatus==='candidate'?`<button data-adaptation-pattern-action="confirm" data-adaptation-pattern-id="${esc(pattern.id)}">Confirm recurring choice</button><button data-adaptation-pattern-action="reject" data-adaptation-pattern-id="${esc(pattern.id)}">This is misleading</button>`:''}${pattern.reviewStatus==='confirmed'?`<button data-adaptation-pattern-action="reject" data-adaptation-pattern-id="${esc(pattern.id)}">Correct this pattern</button>`:''}${pattern.reviewStatus==='rejected'?`<button data-adaptation-pattern-action="reopen" data-adaptation-pattern-id="${esc(pattern.id)}">Reopen for review</button>`:''}</div><small>Current judgement influence: none</small></article>`).join(''):`<div class="empty"><p class="muted">No repeated adaptation choice has enough comparable evidence yet.</p></div>`}
      <p class="muted preference-summary">${adaptationAudit.confirmed} confirmed · ${adaptationAudit.candidate} candidate · ${adaptationAudit.rejected} rejected · 0 influential</p>
