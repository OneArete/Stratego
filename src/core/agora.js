import { applyAdvisorMemory } from './advisor-memory.js?v=0720p1';
...
import { evidenceItem, assessEvidenceDiversity, detectContradictions } from './evidence-integrity.js?v=0720p1';
import { buildDeliberationTrace,buildMinorityReports,summarizeMinorityReports } from './deliberation-trace.js?v=0720p1';
import { applyLongitudinalAdjustments,longitudinalConfidenceAdjustment,verifyLongitudinalEvidence } from './longitudinal-evidence.js?v=0720p1';
import { applyConfirmedBeliefAdjustments } from './belief-system.js?v=0720p1';
import { applyCalibrationToConfidence,verifyCalibrationEvidence } from './calibration-governance.js?v=0720p1';
...
function bodyAdvisor(c,u,h){
  ...
  if(c.soreness==='significant') return opinion('Body','Oppose',92,'Significant soreness makes additional strength loading inappropriate until the physical state is reassessed.',{strength:-1.5,recovery:1.15,walk:.18,focus:.12,connection:.18,direction:.22,'decisive-action':.16},1.2,[],[{domain:'physical',severity:'critical',description:'Significant soreness with proposed strength loading.',reversibility:'difficult'}],[evidenceItem('current_signal','The person reported significant soreness.',{source:'Today signals',reliability:'high'}),evidenceItem('deterministic_rule','Significant soreness blocks additional strength loading.',{source:'Strategos safety policy',reliability:'high'})]);
  const mild=c.soreness==='mild';
  return opinion('Body',mild?'Caution':u.energy>.58?'Support':'Caution',Math.round((.55+u.energy*.35)*100),mild?'Mild soreness reduces the value of high-load work today.':u.energy>.58?'Your reported energy supports productive physical work.':'Your body may benefit more from a lower-cost practice today.',{strength:.35+u.energy*.65-(recent?.2:0)-(mild?.42:0),recovery:.35+(1-u.energy)*.5+(mild?.28:0),walk:.45+(mild?.12:0),focus:.2,connection:.1,direction:.15,'decisive-action':.18},1.1,recent?['Residual fatigue from recent strength work.']:[],mild?[{domain:'physical',severity:'moderate',description:'Mild soreness increases load sensitivity.',reversibility:'easy'}]:[],[evidenceItem('current_signal','Current energy and soreness signals.',{source:'Today signals',reliability:'moderate'}),...(recent?[evidenceItem('person_history','Recent reflected strength practice.',{source:'Journey history',reliability:'limited'})]:[])]);
}
function recoveryAdvisor(c,u){
  ...
  return opinion('Recovery',need>.48?'Caution':'Support',Math.round((.58+Math.abs(need-.5)*.5)*100),need>.48?'Recovery deserves greater weight than intensity today.':'Current sleep, energy and emotional load do not strongly constrain action.',{recovery:.25+need*.8,walk:.35+need*.25,strength:.7-need*.8,focus:.45-need*.35,connection:.28+emotional*.5,direction:.32+need*.1,'decisive-action':.3-need*.2},1.15,[],[],[evidenceItem('current_signal','Sleep, energy, soreness and emotional load.',{source:'Today signals',reliability:'moderate'})]);
}
function mindAdvisor(c,u){
  const focus=c.challenge==='focus'||c.challenge==='work';
  const heavy=c.emotionalLoad==='heavy';
  return opinion('Mind',heavy?'Caution':focus?'Support':'Neutral',heavy?82:focus?84:66,heavy?'High emotional load reduces the value of another cognitively demanding task.':focus?'Your principal challenge requires protected attention.':'There is no strong cognitive constraint in the signals provided.',{focus:heavy?.12:focus?.95:.35,walk:heavy?.62:c.challenge==='mind'?.7:.35,recovery:heavy?.82:.3,strength:heavy?.08:.3,connection:heavy?.68:.25,direction:heavy?.3:.4,'decisive-action':heavy?.15:.35},1,[],heavy?[{domain:'psychological',severity:'moderate',description:'Heavy emotional load may reduce cognitive capacity and increase burden.',reversibility:'easy'}]:[],[evidenceItem('current_signal','Current challenge and emotional load.',{source:'Today signals',reliability:'moderate'})]);
}
function agencyAdvisor(c,u){const heavy=c.emotionalLoad==='heavy';return opinion('Agency',heavy?'Caution':'Support',72,heavy?'The next action should remain deliberately small so it does not become another obligation.':`A ${c.time}-minute commitment is realistic enough to preserve follow-through.`,{strength:heavy?.08:c.time>=15?.6:.1,recovery:heavy?.82:.55,focus:heavy?.18:c.time>=15?.62:.2,walk:.65,connection:heavy?.7:.55,direction:heavy?.3:.5,'decisive-action':heavy?.32:1.3},.9,[],[],[evidenceItem('current_signal','Available time and emotional load.',{source:'Today signals',reliability:'moderate'})])}
function purposeAdvisor(c){const work=c.challenge==='work'||c.challenge==='focus',heavy=c.emotionalLoad==='heavy';return opinion('Purpose',work&&!heavy?'Support':'Neutral',70,work&&!heavy?'Finishing one meaningful task is aligned with today’s stated priority.':heavy?'Purpose does not require adding a demanding task when current burden is already high.':'No single purpose claim dominates today.',{focus:work&&!heavy?.85:.22,connection:c.challenge==='family'?.7:.25,strength:.3,recovery:heavy?.62:.3,walk:.3,direction:work&&!heavy?.5:heavy?.42:1.05,'decisive-action':work&&!heavy?.4:.3},.85,[],[],[evidenceItem('current_signal','Person-stated priority and emotional load.',{source:'Today signals',reliability:'moderate'})])}
function relationshipsAdvisor(c){const relational=c.challenge==='family',heavy=c.emotionalLoad==='heavy';return opinion('Relationships',relational||heavy?'Support':'Neutral',relational?88:heavy?76:61,relational?'The most important pressure today is relational, not physical.':heavy?'Connection may provide support without adding another performance demand.':'No relevant relational conflict was identified.',{connection:relational?1:heavy?.72:.2,recovery:heavy?.55:.25,walk:.3,focus:heavy?.08:.2,strength:heavy?.04:.15,direction:.25,'decisive-action':.25},1,[],[],[evidenceItem('current_signal','Relational priority and emotional load.',{source:'Today signals',reliability:'moderate'})])}
function judgementText(id,d,c){if(c.soreness==='significant'&&id==='recovery')return `A ${d}-minute recovery practice is the safest useful direction while significant soreness is present.`;return {strength:`A ${d}-minute strength practice offers the best current balance of adaptation and continuity.`,recovery:`A ${d}-minute recovery practice appears to offer the highest return today.`,focus:`A protected ${d}-minute focus practice is my best current judgement.`,walk:`A ${d}-minute walk appears to create useful energy without unnecessary cost.`,connection:`A deliberate ${d}-minute act of connection appears more valuable than another performance task today.`,direction:`A ${d}-minute check on direction appears more valuable than another task right now.`,'decisive-action':`A ${d}-minute decisive action appears to offer more than continued deliberation today.`}[id]}
function intention(id){return {strength:'Build strength deliberately.',recovery:'Recover without losing momentum.',focus:'Protect attention. Finish what matters.',walk:'Create energy, not fatigue.',connection:'Be fully present with someone who matters.',direction:'Reconnect action to what actually matters.','decisive-action':'Act before the moment to act passes.'}[id]}
